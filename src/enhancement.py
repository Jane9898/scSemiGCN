import numpy as np
'''
adapted from http://snap.stanford.edu/ne/
'''
...
def network_enhancement(W_in, order, k, alpha):
    ...
    zero_index = np.nonzero(np.sum(np.abs(W_in1), axis=0) > 0) #axis=0
    ...
    W = U @ D @ U.T
    '''
    W = W * (1 - np.eye(W.shape[0])) / np.tile((1-np.diag(W)).reshape(-1, 1), (1, W.shape[0]))#沿x方向
    D = np.diag(DD)
    W = D @ W
    W[W<0] = 0
    W = (W + W.T) / 2
